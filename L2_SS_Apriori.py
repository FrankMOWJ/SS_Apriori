...
def returnItemsWithMinSupport(CandidateSet, transactionList, minSupport, freqSet, transaction_dict: dict, mode, k):
    """
    Function:
    ...
    for idx, item in enumerate(CandidateSet):
        if mode == 'SS-Apriori':
            ...
            if k <= 2:
                min_item = min(item, key=lambda x: len(transaction_dict[x]))
            else:
                min_transaction = 99999
                items = combinations(item, 2)
                min_item = None
                for cur_item in items:
                    cur_item = frozenset(cur_item)
                    if cur_item in transaction_dict and len(transaction_dict[cur_item]) < min_transaction:
                        min_transaction = len(transaction_dict[cur_item])
                        min_item = cur_item


            # 2. only have to scan the transaction that contain the min item
            for transaction_idx in transaction_dict[min_item]:
                num_scan_times += 1
                if item.issubset(transactionList[transaction_idx]):
                    freqSet[item] += 1
                    localSet[item] += 1
                    if k == 2:
                        if item not in L2_transaction_dict:
                            L2_transaction_dict[item] = set()
                            L2_transaction_dict[item].add(transaction_idx)
                        else:
                            L2_transaction_dict[item].add(transaction_idx)
        else:
            # tradition Apriori
...
def runApriori(data_iter, minSupport, minConfidence, mode):
    ...
    oneCSet = returnItemsWithMinSupport(itemSet, transactionList, minSupport, freqSet, L1_transaction_dict, mode, k=1)
    currentLSet = oneCSet
    ...
    while currentLSet != set([]):
        ...
        if k <= 2:
            currentLSet = returnItemsWithMinSupport(
                currentCSet, transactionList, minSupport, freqSet, L1_transaction_dict, mode, k
            )
        else:
            currentLSet = returnItemsWithMinSupport(
                currentCSet, transactionList, minSupport, freqSet, L2_transaction_dict, mode, k
            )
        k = k + 1
    ...
    addition_storage = 0
    for key, value in L2_transaction_dict.items():
        # for transaction in value:
        #     addition_storage += len(transactionList[transaction])
        addition_storage += len(value)
    
    print(f'additional transaction storage: {addition_storage}')
    def getSupport(item):
